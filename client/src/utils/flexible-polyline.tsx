...
export function decodeHerePolyline(encoded: string): number[][] {
  let index = 0;
  const result: PolylineResult = {
    precision: 5,
    thirdDim: 0,
    thirdDimPrecision: 0,
    polyline: [],
  ...
  while (index < encoded.length) {
    ...
    const point: number[] = [lastLat / factorDegree, lastLng / factorDegree];
...
function decodeUnsignedVarint(encoded: string, startIndex: number): DecodedVarint {
  let result = 0;
  let shift = 0;
  let byte = null;
...
function decodeSignedVarint(encoded: string, startIndex: number): DecodedVarint {
